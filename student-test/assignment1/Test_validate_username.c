...
void test_validate_my_username()
{
    ...
    char *name_from_config_file = malloc(1);

    name_from_config_file = malloc_username_from_conf_file();

    TEST_ASSERT_EQUAL_STRING_MESSAGE(name_from_my_username, name_from_config_file, "Username is not equal");

    free(name_from_config_file);
}
